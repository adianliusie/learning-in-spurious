...
from ..models.models import TransformerModel, PromptFinetuning
from ..utils.general import save_json, load_json
from ..utils.torch import set_rand_seed
from ..loss.cross_entropy import CrossEntropyLoss
from ..loss.kl_logit_reg import KlLogitRegLoss
from ..loss.l2_weights import L2WeightRegLoss
...
class Trainer(object):
    ...
    def setup_helpers(self, args: namedtuple):
        ...
        if args.prompt_finetuning:
            assert len(args.label_words) == args.num_classes, "for prompt finetuning, need a label word for each class"
        ...
        elif args.loss == 'kl-logits':
             self.model_loss = KlLogitRegLoss(self.model, alpha=args.loss_args[0], vocab=False)
        elif args.loss == 'kl-vocab':
             self.model_loss = KlLogitRegLoss(self.model, alpha=args.loss_args[0], vocab=True)
        elif args.loss == 'l2-weights':
             self.model_loss = L2WeightRegLoss(self.model, alpha=args.loss_args[0])
